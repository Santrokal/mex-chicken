...
const PeriPeriFamilyMeals = ({ onAddToCart, searchTerm }) => {
  ...
  useEffect(() => {
    fetch("http://localhost:8000/products")
      .then((res) => res.json())
      .then((data) => {
        const periperifamilymealsProducts = data.filter(
          (product) => product.category === "Peri Peri Family Meals"
        );
        setProducts(periperifamilymealsProducts);
      })
...
export default PeriPeriFamilyMeals;

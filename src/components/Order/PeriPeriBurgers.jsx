...
const PeriPeriBurgers = ({ onAddToCart, searchTerm }) => {
  ...
  useEffect(() => {
    fetch("http://localhost:8000/products")
      .then((res) => res.json())
      .then((data) => {
        const periperiburgersProducts = data.filter(
          (product) => product.category === "Peri Peri Burgers"
        );
        setProducts(periperiburgersProducts);
      })
...
export default PeriPeriBurgers;

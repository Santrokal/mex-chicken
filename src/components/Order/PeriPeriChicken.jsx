...
const PeriPeriChicken = ({ onAddToCart, searchTerm }) => {
  ...
  useEffect(() => {
    fetch("http://localhost:8000/products")
      .then((res) => res.json())
      .then((data) => {
        const periperichickenProducts = data.filter(
          (product) => product.category === "Peri Peri Chicken"
        );
        setProducts(periperichickenProducts);
      })
  ...
  return (
    <div
      className="category-products flex flex-col gap-2 gap-y-3 mt-3 rounded-sm2"
      id="5">
      <div className="category-name  text-red font-Avertastd capitalize pb-3">
        Peri Peri Chicken
      </div>
              ...
              <div className="product-name w-full">
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  <button
                    ...
                    className="bg-red text-base1 px-8 py-2 rounded-md text-lg font-semibold hover:bg-red transition">
                    Add
                  </button>
                </div>
              </div>

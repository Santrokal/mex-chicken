...
const FamilyBucket = ({ onAddToCart, searchTerm }) => {
  ...
  useEffect(() => {
    fetch("http://localhost:8000/products")
      .then((res) => res.json())
      .then((data) => {
        const familybucketProducts = data.filter(
          (product) => product.category === "Family Bucket"
        );
        setProducts(familybucketProducts);
      })
